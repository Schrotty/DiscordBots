...
class AdminCommands(Cog):
    ...
    async def list_banned_channels(self, ctx: Context):
        ...
        if len(result) > 0:
            return await ctx.send(Template.LIST_BANNED_CHANNEL.format(
    ...
    async def ban_channel(self, ctx: Context, channel: TextChannel):
        if Database.execute('SELECT channel FROM level_banned_channel WHERE channel=%s', (channel.id,)).rowcount == 0:
