...
class Rank(Cog):
        ...
        with db_session:
            ...
            if not BannedChannel.select(lambda c: c.channel == channel.id).exists():
                ...
                if user_info is None:
                    user_info = UserInfo(id=str(author.id), name=author.name, exp=0, level=0,
                                         exp_time=0, avatar_url=str(author.avatar_url))
                ...
                if (user_info.exp_time + COOLDOWN <= time.time()) or os.getenv('PAPERBOT.ENVIRONMENT') == 'develop':
                    ...
                    if user_info.exp >= level_up:
                        ...
                        level_reward: LevelReward = LevelReward.select(
                            lambda r: r.reward_level == user_info.level).first()
                        ...
                        if level_reward is not None:
                            ...
                            # DO NOT SIMPLIFY!
                            old_roles = list(select(r.reward_role for r in LevelReward)
                                             .where(lambda r: r.reward_level < user_info.level and r.reward_level > 1))
    ...
    async def cmd_rank(self, ctx: commands.Context) -> None:
        ...
        with db_session:
            ...
            if user_info is None:
                user_info = UserInfo(id=str(author.id), name=author.name, exp=0, level=0,
                                     exp_time=int(time.time()), avatar_url=str(author.avatar_url))
